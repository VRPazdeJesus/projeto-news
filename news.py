...
def main():
    ...
    if resposta.status_code == 200:
        ...
        while escolha != 0:
            ...
            print("0 - Para sair")
            try:
                escolha = int(input("Digite o número referente a categoria que você deseja baixar!"))
            except:
        ...
        print("Possivelmente algum problema na URL")
...
def buscar_all(dados):
    ...
    if len(titulo) == 0:
        print("Não foram encontradas notícias nesta categoria")
    else:
...
def exportar_csv(titulo, link, nome):
    tela = pandas.DataFrame({'Titulo':titulo, 'Link':link})
